...
public class CircleTest {
    ...
    public void testDistanceIsZeroWhenIntersects() {
        Circle circle = circle(0, 0, 1);
        assertTrue(circle.distance(Geometries.box(0, 1, 0, 1)) == 0);
    }

    @Test
    public void testIntersects2() {
        Circle circle = circle(0, 0, 1);
        assertTrue(circle.distance(Geometries.box(0, 1.1, 0, 1.1)) != 0);
    }

    @Test
    public void testIntersects3() {
        Circle circle = circle(0, 0, 1);
        assertTrue(circle.distance(Geometries.box(1, 1, 1, 1)) != 0);
    }

    @Test
    public void testIntersectsReturnsTrue() {
        assertTrue(circle(0, 0, 1).intersects(Geometries.box(0, 0, 1, 1)));
    }

    @Test
    public void testIntersectsReturnsFalse() {
        assertFalse(circle(0, 0, 1).intersects(Geometries.box(10, 10, 11, 11)));
    }
